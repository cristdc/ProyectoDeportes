...
const HomeAdmin = () => {
  ...
  const filteredRaces = races.races
    ...
    if (window.confirm("¿Estás seguro de que quieres eliminar esta carrera?")) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/races/${raceId}`,
          {
            method: "DELETE",
            credentials: "include",
          }
        ...
        // Corregido: Filtrar dentro de races.races
        setRaces((prevState) => ({
          ...prevState,
          races: prevState.races.filter((race) => race._id !== raceId),
        }));
      } catch (error) {
  ...
  const handleDownloadCSV = async (raceId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/races/${raceId}/download-csv`,
        {
          credentials: "include",
        }
      );
