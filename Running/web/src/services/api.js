...
export const apiService = {
  ...
  getParticipations: async (filters = {}) => {
    try {
      ...
      const endpoint = `/registrations/user${
  ...
  getRegistrationId: async (raceId) => {
    try {
      ...
      if (!registration) {
        throw new Error(
          "No se encontró una inscripción activa para esta carrera"
        );
      }
  ...
  unregisterFromRace: async (raceId) => {
    try {
      ...
      const response = await fetchWithAuth(
        `/registrations/${registration._id}/cancel`,
        {
          method: "PUT",
        }
      );
  ...
  downloadGpxFile: async (raceId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/races/${raceId}/gpx`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("userToken")}`,
          },
  ...
  toggleRaceRegistration: async (raceId) => {
    try {
      ...
      if (existingRegistration) {
        ...
        const response = await fetchWithAuth(
          `/registrations/${existingRegistration._id}/cancel`,
          {
            method: "PUT",
          }
        );
